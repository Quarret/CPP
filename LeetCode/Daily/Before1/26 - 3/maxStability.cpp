...
int maxStability(int n, vector<vector<int>>& edges, int k) {
    UnionFind all_dsu(n);
    UnionFind must_dsu(n);
    int min_s = INT_MAX, max_s = 0;
    for (auto &e : edges) {
        int u = e[0], v = e[1], s = e[2], must = e[3];
        if (must && !must_dsu.merge(u, v)) return -1;
        all_dsu.merge(u, v);
        min_s = min(min_s, s);
        max_s = max(max_s, s); 
    }
    
    if (all_dsu.cc > 1) return -1;
    ...
    auto check = [&](int low) -> bool {
        ...
        int left_k = k;
        for (auto &e : edges) {
            if (left_k == 0 || dsu.cc == 1) {
                break;
            }
            int u = e[0], v = e[1], s = e[2], must = e[3];
            if (!must && s < low && 2 * s >= low && dsu.merge(u, v)) {
                left_k--;
            }
        }
    ...
    int left = min_s, right = 2 * max_s + 1;
    while (left + 1 < right) {
